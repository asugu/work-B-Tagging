...
import numpy as np
import math
from flash_attn.modules.mha import MHA#flash_attn_qkvpacked_func, flash_attn_func
...
def create_padding_mask_(x, jet_inputs):
    ...
    padding_mask = padding_mask.view(N,1,1,C)
...
class PairEmbed(nn.Module):
    def __init__(self, num_heads, in_channels, out_channels):
        ...
        self.conv2 = nn.Conv2d(out_channels, out_channels, kernel_size=2, stride=2, padding=0)  # for pad 50 = 3 3 3
    ...
    def forward(self, four_momenta):
        ...
        input_tensor = x.view(N*self.num_heads,8,8)
...
class ParticleBlock(nn.Module):
    def __init__(self,embed_dim, num_heads=4,ffn_ratio=2,
        ...
        self.attn = MHA(#nn.MultiheadAttention(
            embed_dim,
            num_heads,
    ...
    def forward(self, x, padding_mask=None, attn_mask=None):
       ...
       # print(padding_mask.shape)
        x = self.attn(x, x, x)[0]#key_padding_mask=padding_mask, attn_mask = attn_mask  # (seq_len, batch, embed_dim)
...
class ClassBlock(nn.Module):
    ...
    def forward(self, x, x_cls=None, padding_mask=None):
        ...
        x = self.attn(x_cls, u, u)[0]  # (1, batch, embed_dim)   key_padding_mask=padding_mask
