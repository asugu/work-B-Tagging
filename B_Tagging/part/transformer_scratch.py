...
class JetParticleDataset_part(torch.utils.data.Dataset):
    def __init__(self, df, device='cpu', eval=False, transform=None, scaler=None, checkpoint=True):
        ...
        self.particle_features = df[['track_d0', 'track_dz', 'track_d0_sig', 'track_dz_sig', 'track_pt', 'track_pid', 'track_charge']]
        self.four_momenta = df[['track_E', 'track_px', 'track_py', 'track_pz']]
        ...
        if not checkpoint:
            if not self.eval:
                ...
                self.df[self.four_momenta.columns] = self.df[self.four_momenta.columns].apply(lambda x: [self.scaler.fit_transform(np.array(arr).reshape(-1, 1)).flatten() for arr in x])
            ...
            if not self.eval:
                self.df[self.four_momenta.columns] = self.df[self.four_momenta.columns].apply(lambda x: [((np.array(arr).reshape(-1, 1) - self.scaler[0]) / self.scaler[1]).flatten() for arr in x])
            else:
                self.df[self.four_momenta.columns] = self.df[self.four_momenta.columns].apply(lambda x: [((np.array(arr).reshape(-1, 1) - self.scaler[0]) / self.scaler[1]).flatten() for arr in x])
    ...
    def __getitem__(self, idx):
        ...
        return jet_inputs, particle_inputs, four_momenta, label, flavor
...
class PairEmbed(nn.Module):
    def __init__(self, num_heads, in_channels, out_channels):
        ...
        self.conv1 = nn.Conv2d(in_channels, out_channels, kernel_size=4, stride=1, padding=0)   # for pad 50 = 3 3 2
        self.conv2 = nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=0)  # for pad 50 = 3 3 3
    ...
    def forward(self, four_momenta):
        ...
        input_tensor = x.view(N*self.num_heads,11,11)
...
class MLP(nn.Module):
    def __init__(self, input_size, output_size=1, n_hidden_layers = 2, nnodes=32):
        super().__init__()
